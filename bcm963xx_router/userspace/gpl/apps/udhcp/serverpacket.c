 ...
 */
static int AEI_is_in_range(int vendor, u_int32_t req_align)
{
...
/* This function is written for XXXXX DHCP option 67 requirement
...
#if defined(AEI_VDSL_DHCP_LEASE) || defined(SUPPORT_GPL_2) || defined(NOT_USED_6)
...
int sendOffer(struct dhcpMessage *oldpacket)
{
...
#if defined(SUPPORT_GPL_1) //add william 2012-1-11
	    /* */
        int vendor;
	...
	if(!static_lease_ip) {
		...
		* we than need to assign a new free IP address to this client */
		if (lease && !reservedIp(cur_iface->static_leases, lease->yiaddr) && AEI_is_in_range(2,lease->yiaddr))
#else
...
#endif
#endif
#if defined(AEI_VDSL_DHCP_AUTO_RESERVATION) ||  defined(AEI_VDSL_DHCP_AUTO_RESERVATION)
			if (!packet.yiaddr)
...
#if defined(SUPPORT_GPL_2) && defined(AEI_VDSL_DHCP_LEASE)
