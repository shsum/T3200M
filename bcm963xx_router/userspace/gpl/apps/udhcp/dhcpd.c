...
#if defined(AEI_VDSL_CUSTOMER_DHCP_OPTION60) && defined(NOT_USED_1)
	...
	while(1) { /* loop until universe collapses */
		...
		if (FD_ISSET(msg_fd, &rfds)) {
            ...
            case CMS_MSG_DHCPD_RELOAD:
					...
					read_leases(server_config.lease_file);
               break;
#if defined(SUPPORT_GPL_1) && !defined(AEI_VDSL_DHCP_LEASE)
            ...
            case CMS_MSG_GET_LEASE_TIME_REMAINING:
               if (msg->dataLength == sizeof(GetLeaseTimeRemainingMsgBody)) {
					   ...
					   if (cur_iface != NULL) {
						   ...
						   if (lease != NULL) {
#if defined (SUPPORT_GPL_2) || defined(NOT_USED_2)
								//static leases
...
#ifdef NOT_USED_6 //add william 2012-4-25
