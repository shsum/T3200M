...
burn_in_init ()
{
    ...
    # for two bhr5c board connect together
    if [ -n "$(cfecmd -g BoardID 2> /dev/null | grep -e "BHR5C_M6240" -e "T3200" -e "C3000")" ]; then
        echo "BHR5C_M6240 or BHR5C_M6240L or T3200 or C3000 no have moca wan, skip moca WAN config"
    else
        echo "set moca WAN as 2.0"
        mocap moca0 set --c4_moca20_en 1 --restart
    fi
