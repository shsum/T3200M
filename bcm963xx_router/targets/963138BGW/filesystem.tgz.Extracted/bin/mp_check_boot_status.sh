...
if [[ ${BoardID} = "T3200B" || ${BoardID} = "C3000" || ${BoardID} = "T3200" ]]; then
    NO_MOCA_WAN=1
    NO_MOCA_LAN=1
fi

if [[ ${BoardID} = "T3200" || ${BoardID} = "T3200M" ]]; then
   PCA9555=0
fi
...
check_main()
{
    ...
    # 9. Check MoCA LAN
    if [ $NO_MOCA_LAN == 0 ]; then
        if [ $NO_MOCA_WAN == 1 ] ;  then
            check_moca 0
        else
            check_moca 1
        fi
    ...
    CheckStatus=$?
    if [ $CheckStatus -eq $FAIL ] ; then
        if [[ "${BoardID}" = "T3200M" || "${BoardID}" = "T3200" ]]; then
