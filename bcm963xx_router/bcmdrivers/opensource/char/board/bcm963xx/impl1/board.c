...
#if defined(AEI_VDSL_CHIP_D0_CHECK)
int  CPURevId=0;
...
void btnHook_mfg(unsigned long timeInMs, unsigned long param);
#endif
...
#elif defined(SUPPORT_GPL_2)
#if defined(NOT_USED_25) || defined(SUPPORT_GPL_4)
...
#if   defined(CONFIG_BCM_6802_MoCA)
#if defined(AEI_MULT_BOARD_ID_IN_ONE_FIRMWARE)
...
int kerSysGetMacAddresses( unsigned char *pucaMacAddr, unsigned int num_addresses, unsigned long ulId )
{
...
#endif	
    {
#if defined(SUPPORT_GPL_6)
    /*
     *  1. It is used to keep using WAN MAC address is same as before while upgrade XXXXX SDK3/SDK6 image to SDK12.
     ...
     *  5. For other XXXXX project (fox example T2200H), my suggestion is that using WAN interface(atm0/ptm0.1/ewan0.1) is B + 1, and atm1 = B + 2.
...
#if defined(SUPPORT_GPL_1) && !defined(AEI_VDSL_DUAL_IMAGE)
    char *p;
#endif
    char *tagFs = imagePtr;
...
static int aei_getNvramValueByParaName(AEI_NVRAM_PARAM_INDEX pIndex, char* parValue, int len)
{
    ...
    case AEI_NVPARAM_WLANFEATURE:
        *parValue = nvramData.wlanParams[NVRAM_WLAN_PARAMS_LEN-1];
        break;
#if defined(AEI_DETECT_BOARD_ID)
...
#endif
#if defined(AEI_VDSL_SECURITY_LEVEL_3)
#if defined(AEI_VDSL_SECURITY_LEVEL_3_TR)
        ...
        break;
         ...
         case BOARD_IOCTL_PRINT_NVRAM:
			//if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
            {
             ...
             break;
#endif
#if defined(AEI_VDSL_UPGRADE_DUALIMG_HISTORY_SPAD) || defined(AEI_CUSTOMER_REVERT_FIRMWARE_CONFIG)
...
#endif
#if defined(AEI_TWO_IN_ONE_FIRMWARE) && defined(NOT_USED_6)
...
#if defined(AEI_FACTORY_TEST)
        if (isInManuMode()) {
            registerPushButtonPressNotifyHook(PB_BUTTON_1, btnHook_mfg, 0);
        }
#endif
    }
#if defined(WIRELESS)
...
#if defined(NOT_USED_2) || defined(SUPPORT_GPL_2) || defined(SUPPORT_BACKUP_PSI)
...
#elif defined(SUPPORT_GPL_2)
/* XXXXX requirement:  If reset button is held up to 10 seconds, just reboot.  If held more than 10 seconds restore default
*/
static void AEI_resetBtnTimerFunc(unsigned long data)
{
#if defined(AEI_63168_CHIP)
...
#if defined(AEI_VDSL_DUAL_IMAGE) || defined(AEI_VER2_DUAL_IMAGE) || defined(AEI_FACTORY_TEST)
...
void btnHook_mfg(unsigned long timeInMs, unsigned long param) {
    int status = -1;
    status = AEI_BUTTON_PRESS;
    kerSysSendtoMonitorTask(AEI_MSG_NETLINK_WPS_BUTTON, (char *)&status, sizeof(status));
}
#endif
...
#if defined(SUPPORT_GPL_1) && defined(AEI_63138_CHIP)
